...
def process_score_chunk(
    anchor_date: pd.Timestamp,
    train_start_date: pd.Timestamp,
    train_end_date: pd.Timestamp,
    chunk_dates: list[pd.Timestamp],
    df_ml: pd.DataFrame | None = None,
    df_price: pd.DataFrame | None = None,
    args=None,
    features: list[str] | None = None,
    ...
    try:
        ...
        if use_knn:
            fill_values = X_train.median(axis=0, skipna=True)
            fill_values = fill_values.replace([np.inf, -np.inf], np.nan).fillna(0.0).astype("float32")
            X_train_knn = X_train.fillna(fill_values)
            scaler = StandardScaler()
            X_train_scaled = scaler.fit_transform(X_train_knn.to_numpy(dtype=np.float32))
            curr_k = min(int(getattr(args, "knn_neighbors", 50)), len(X_train) - 1)
            if curr_k >= 1:
                model_knn = KNeighborsRegressor(n_neighbors=curr_k, weights="distance", n_jobs=1)
                model_knn.fit(X_train_scaled, y_train.to_numpy(dtype=np.float32))
            else:
                model_knn = None
                scaler = None
                fill_values = None
        ...
        for target_date in chunk_dates:
            ...
            if use_knn and model_knn is not None and scaler is not None and fill_values is not None:
                X_test_knn = X_test.fillna(fill_values)
                X_test_scaled = scaler.transform(X_test_knn.to_numpy(dtype=np.float32))
                pred_knn = model_knn.predict(X_test_scaled)
